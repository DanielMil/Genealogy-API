...
GEDCOMerror createGEDCOM(char* fileName, GEDCOMobject** obj) {
	...
	//Validate the file. If there is an error, all memory allocated up to now is free'd and message is returned.
	...
	for (int i = 0; i < numOfFamIndRecords; i++)
		free(linker[i].xrefId);
	free(linker);
...
char* printGEDCOM(const GEDCOMobject* obj) {
	...
	gedcom = realloc(gedcom, sizeof(char) * (strlen(gedcom) + strlen(obj->header->source) + strlen(version) + 70));
	strcat(gedcom, "\n-----HEADER-----\n");
	...
	gedcom = realloc(gedcom, sizeof(char) * (strlen(gedcom) + strlen(obj->submitter->submitterName) + strlen(obj->submitter->address) + 70));
	strcat(gedcom, "\n-----Submitter-----\n");
	strcat(gedcom, "Name: ");
	...
	while ((temp = nextElement(&families)) != NULL) {
		...
		gedcom = realloc(gedcom, sizeof(char) * (strlen(gedcom) + strlen(next) + 20));
		strcat(gedcom, next);
		strcat(gedcom, "\n\n");
	}
...
ErrorCode validateGEDCOM(const GEDCOMobject* obj) {
	...
	if (!obj->header->submitter || !obj->header->gedcVersion || !obj->header->encoding) {
		error = INV_HEADER;
		return error;
	}
	...
	while ((tempIndividual = (Individual*)nextElement(&individuals)) != NULL) {
		...
		if (strlen(tempIndividual->givenName) > MAX_VALUE_LENGTH || strlen(tempIndividual->surname) > MAX_VALUE_LENGTH) {
			error = INV_RECORD;
			return error;
		}
		...
		while ((tempEvent = (Event*)nextElement(&eventsIter)) != NULL) {
			if (strlen(tempEvent->date) > MAX_VALUE_LENGTH || strlen(tempEvent->place) > MAX_VALUE_LENGTH) {
				error = INV_RECORD;
				return error;
			}
		}
...
List getDescendantListN(const GEDCOMobject* familyRecord, const Individual* person, unsigned int maxGen) {
	...
	List descendantsNew = initializeList(&printGeneration, &deleteGeneration, &compareGenerations);
	...
	for (int i = 0; i < maxGen; i++) {
		newGeneration[i] = initializeList(&printIndividual, &dummyDelete, &sortForDescendants);
...
List getAncestorListN(const GEDCOMobject* familyRecord, const Individual* person, int maxGen) {
	...
	List ancestorsNew = initializeList(&printGeneration, &deleteGeneration, &compareGenerations);
...
char* indToJSON(const Individual* ind) {
	...
	JSON = malloc(sizeof(char) * (strlen(ind->givenName) + strlen(ind->surname) + 30));
...
GEDCOMobject* JSONtoGEDCOM(const char* str) {
	...
	if ((tempSource = getJSONValue(str, "source")) == NULL)
		return NULL;

	strcpy(obj->header->source, tempSource);

	if ((tempEncoding = getJSONValue(str, "encoding")) == NULL)
		return NULL;
	...
	if ((tempVersion = getJSONValue(str, "gedcVersion")) == NULL)
		return NULL;

	obj->header->gedcVersion = atof(tempVersion);

	if ((tempSubmitterName = getJSONValue(str, "subName")) == NULL)
		return NULL;
	strcpy(obj->submitter->submitterName, tempSubmitterName);

	if ((tempAddress = getJSONValue(str, "subAddress")) == NULL)
		return NULL;
...
char* iListToJSON(List iList) {
	...
	while ((tempIndividual = (Individual*)nextElement(&iter)) != NULL) {
		...
		JSON = realloc(JSON, sizeof(char) * (strlen(JSON) + strlen(indToJSON(tempIndividual)) + 6));
		strcat(JSON, indToJSON(tempIndividual));

		mainListCounter++;
		if (mainListCounter != listSizeMainList)
			strcat(JSON, ",");
	}
...
char* printEvent(void* toBePrinted) {
	...
	char *toString = malloc(sizeof(char) * (strlen(toPrint->date) + strlen(toPrint->place) + strlen(toPrint->type) + 34));
	strcpy(toString, "\nType: ");
	...
	while ((temp = nextElement(&fields)) != NULL) {
		...
		toString = realloc(toString, sizeof(char) * (strlen(toString) + strlen(next) + 20));
...
int compareIndividuals(const void* first, const void* second) {
	...
	char * x = malloc(sizeof(char) * (strlen(a->givenName) + strlen(a->surname) + 3));
	char * y = malloc(sizeof(char) * (strlen(b->givenName) + strlen(b->surname) + 3));
...
char* printIndividual(void* toBePrinted) {
	...
	char *string = malloc(sizeof(char) * (strlen(person->givenName) + strlen(person->surname) + 68));
	...
	while ((temp = nextElement(&fields)) != NULL) {
		strcat(string, "\n");
		next = printField(temp);
		string = realloc(string, sizeof(char) * (strlen(string) + strlen(next) + 20));
		strcat(string, next);
	}
...
char* printField(void* toBePrinted) {
	...
	char *string = malloc(sizeof(char) * (strlen(a->tag) + strlen(a->value)) + 20);
	strcpy(string, "Tag: ");
	strcat(string, a->tag);
	strcat(string, " Value: ");
