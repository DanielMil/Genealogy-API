...
char *createGEDCOMJSON(char *string) {
  ...
  Family *tempFam = NULL;
  while ((tempFam = nextElement(&iterFam)) != NULL) numOfFamilies++;

  ListIterator iterInds = createIterator(obj->individuals);
  Individual *tempInd = NULL;
  while ((tempInd = nextElement(&iterInds)) != NULL) numOfIndividuals++;
...
char *createGEDCOMIndividualJSON(char *fileName) {
  ...
  while ((tempInd = nextElement(&iterInds)) != NULL) {
    individual =
        realloc(individual, sizeof(char) * (strlen(individual) +
                                            strlen(tempInd->givenName) +
                                            strlen(tempInd->surname) + 100));
    ...
    while ((tempFam = nextElement(&iterFam)) != NULL) {
      if (tempInd == tempFam->husband || tempInd == tempFam->wife) {
        if (tempFam->wife && tempFam->husband) familySize++;
...
char *descendantsInterface(char *filename, char *givenName, char *surname,
                           int maxGen) {
  char *JSON = NULL;
  ...
  while ((tempIndividual = (Individual *)nextElement(&iter)) != NULL) {
    if (strcmp(tempIndividual->givenName, givenName) == 0 &&
        strcmp(tempIndividual->surname, surname) == 0) {
...
char *ancestorsInterface(char *filename, char *givenName, char *surname,
                         int maxGen) {
  char *JSON = NULL;
...
GEDCOMerror validateFile(char **tempBuffer, int numOfLines) {
  ...
  if (strcmp(firstLevel, "0") == 0) {
    ...
    while (level != 0) {
      ...
      if (strcmp(tag, "SUBM") == 0) submitterReferenceInHeadRecord = true;

      if (strcmp(tag, "SOUR") == 0) sourceInHeader = true;

      if (strcmp(tag, "VERS") == 0) versionInHeader = true;

      if (strcmp(tag, "CHAR") == 0) encodingInHeader = true;
  ...
  if (!submitterReferenceInHeadRecord || !sourceInHeader || !encodingInHeader ||
      !versionInHeader) {
    message.line = -1;
    message.type = INV_HEADER;
  ...
  for (int i = 0; i < numOfLines - 1; i++) {
    ...
    if (tempBuffer[i + 1]) charNextLevel = getLevel(tempBuffer[i + 1]);
    ...
    if (level == 0 && (strcmp(tag, "HEAD") != 0) &&
        (strcmp(tag, "TRLR") != 0) && (strcmp(value, "INDI") != 0) &&
        (strcmp(value, "FAM") != 0) && (strcmp(value, "SUBM") != 0)) {
      message.line = i + 1;
      message.type = INV_RECORD;
...
char *getTag(char *toParse) {
  if (strlen(toParse) <= 2) return NULL;
...
char *getValue(char *toParse) {
  ...
  while (token != NULL) {
    ...
    if (token != NULL) strcat(value, delim);
  }
...
char *getSurname(char *toParse) {
  ...
  while (token != NULL) {
    ...
    if (token != NULL) strcat(surname, delim);
  }

  if (surname[strlen(surname) - 1] == '/') surname[strlen(surname) - 1] = '\0';
...
void loadHead(GEDCOMobject *obj, char **tempBuffer) {
  ...
  while (1) {
    if (getLevelInt(tempBuffer[i]) == 0) break;
    ...
    if (strcmp(tempTag, "SOUR") == 0) {
      ...
      free(tempValue);
      free(tempTag);
    } else if (strcmp(tempTag, "VERS") == 0 &&
               getLevelInt(tempBuffer[i]) == 2) {
      tempValue = getValue(tempBuffer[i]);
  ...
  if (!loadedEncoding) obj->header->encoding = UTF8;
  if (!loadedVersion) obj->header->gedcVersion = 1.0;
  if (!loadedSource) strcpy(obj->header->source, "");
}
...
void loadIndividual(GEDCOMobject *obj, char **tempBuffer, int startIndex,
                    Individual *recordPtr) {
  int i = startIndex + 1;
  ...
  while (1) {
    ...
    if (strcmp(tempTag, "NAME") == 0 &&
        tokenCount >= 4) {  // Name with first and last name on same line
      ...
      free(tempTag);
      free(tempValue);
    } else if (strcmp(tempTag, "NAME") == 0 &&
               tokenCount == 3) {  // Name with only first name
      ...
      while (1) {
        ...
        if (strcmp(tempTag, "GIVN") == 0) {
          ...
          recordPtr->surname =
              getGivenName(tempBuffer[j]);  // In this case, given name is the
                                            // surname based on how the parser
                                            // works.
          free(tempValue);
          free(tempTag);
        } else {
      ...
      // Create new field
      if (tokenCount >= 3 && getLevelInt(tempBuffer[i]) == 1 &&
          strcmp(tempTag, "FAMC") != 0 && strcmp(tempTag, "FAMS") != 0) {
...
Individual *createIndividualRecord() {
  ...
  newIndividual->families =
      initializeList(&printFamily, &dummyDelete, &compareFamilies);
...
Family *createFamilyRecord() {
  ...
  newFamily->otherFields =
      initializeList(&printField, &deleteField, &compareFields);
  newFamily->children =
      initializeList(&printIndividual, &dummyDelete, &compareIndividuals);
...
void loadFamilies(GEDCOMobject *obj, char **tempBuffer, int startIndex,
                  Family *recordPtr) {
  int i = startIndex + 1;
  int tokenCount = 0;
  char *tempLevel = NULL;
  ...
  while (1) {
    ...
    // Create new field
    if (tokenCount >= 3 && strcmp(tempLevel, "1") == 0 &&
        strcmp(tempTag, "HUSB") != 0 && strcmp(tempTag, "WIFE") != 0 &&
        strcmp(tempTag, "CHIL") != 0) {
      createNewField(&recordPtr->otherFields, tempBuffer[i]);
      ...
      while (1) {
        ...
        if (currentLevelChar != NULL) free(currentLevelChar);

        if (nextLevelChar != NULL) free(nextLevelChar);
...
void linkIndividual(GEDCOMobject *obj, char **array, Link *linker,
                    int numOfLines, int numOfFamIndRecords) {
  char *tempTag = NULL;
  char *tempValue = NULL;
  int j = 0;
  ...
  for (int i = 0; i < numOfLines; i++) {
    ...
    if (tempTag[0] == '@' &&
        (strcmp(tempValue, "INDI") == 0 || strcmp(tempValue, "FAM") == 0)) {
        ...
        if (strcmp(tempTag, "FAMC") == 0 || strcmp(tempTag, "FAMS") == 0) {
            ...
            if (strcmp(linker[k].xrefId, tempXrefId) == 0) {
              for (int a = 0; a < numOfFamIndRecords; a++) {
                if (strcmp(linker[a].xrefId, tempValue) == 0) {
                  insertFront(&((Individual *)linker[k].ptr)->families,
                              linker[a].ptr);
                }
              }
            }
          ...
          for (int k = 0; k < numOfFamIndRecords; k++) {
            if (strcmp(linker[k].xrefId, tempXrefId) == 0) {
              for (int a = 0; a < numOfFamIndRecords; a++) {
                if (strcmp(linker[a].xrefId, tempValue) == 0) {
                  insertFront(&((Family *)linker[k].ptr)->children,
                              linker[a].ptr);
                }
              }
            }
...
void addDescendantsN(List *list, Family *familyToSearch, int maxIter,
                     int currIter) {
  Individual *toAdd = NULL;
  Individual *tempIndividual = NULL;
  Family *temp = NULL;
...
void addAncestorsN(List *list, Family *familyToSearch, int maxIter,
                   int currIter) {
  // Individual *toAdd = NULL;
  Family *temp = NULL;
  Individual *tempChild = NULL;
...
Individual *copyIndividual(Individual *tempIndividual) {
  ...
  toAdd->givenName =
      malloc(sizeof(char) * strlen(tempIndividual->givenName) + 1);
...
void writeIndividuals(List individuals, FILE *savePtr, Link *linker,
                      int numOfLinks) {
  Individual *tempInd = NULL;
  Event *tempEvent = NULL;
  Field *tempField = NULL;
...
void writeFamilies(List families, FILE *savePtr, Link *linker, int numOfLinks,
                   int numOfFams) {
  Family *tempFam = NULL;
  Event *tempEvent = NULL;
  Individual *tempIndividual = NULL;
  ...
  while ((tempFam = (Family *)nextElement(&famsIter)) != NULL) {
    ...
    while ((tempIndividual = (Individual *)nextElement(&childrenIter)) !=
           NULL) {
      for (int i = 0; i < numOfLinks; i++) {
